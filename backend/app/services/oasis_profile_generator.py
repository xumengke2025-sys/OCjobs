...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            # 使用 ZepToolsService 进行搜索
            search_result = self.zep_tools.search_graph(
            ...
            for node in search_result.nodes:
                if node.get("summary"):
                    results["node_summaries"].append(node["summary"])
                if node.get("name") and node["name"] != entity_name:
                    results["node_summaries"].append(f"相关实体: {node['name']}")
    ...
    def _generate_profile_with_llm(
        ...
        try:
            ...
            logger.warning(f"LLM生成人设失败: {e}, 使用规则生成")
